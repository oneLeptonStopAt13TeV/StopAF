...
#define OBSERVABLE_FOR_FIT "category"
//#define OBSERVABLE_FOR_FIT "yield"
...
FitResult  doFit(const FitSetup& setup, string conditions, string fname=string(""))
{
    ...
    TH1F* h_ttbar  =  GetHisto(fin,region,"ttbar",varname, mc_norm_ttbar);
    TH1F* h_stop  =  GetHisto(fin,region,"stop",varname, mc_norm_ttbar);
    TH1F* h_data = (TH1F*) h_ttbar->Clone("data_obs");
    h_data->Add(h_stop);

    TFile* fout = new TFile("fit.root","RECREATE");
