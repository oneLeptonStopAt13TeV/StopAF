...
// njets; nof jets
// jets: vector of TLV
// btag: array containing booleans - True if jet is b-tagged
double Resolution::GetChi2(int n_jets, TLorentzVector* jets, bool* btag)
{
  ...
  */
  ...
  for ( int i=0; i<n_jets; ++i )
    for ( int j=i+1; j<n_jets; ++j )
    {
      ...
      minimizer.SetParameter(5 , "m12"    , jets[i].M2() , 1 , 0 , 0);
      minimizer.SetParameter(6 , "m22"    , jets[j].M2() , 1 , 0 , 0);
	...
	continue;
      ...
      double c2 = Resolution::fc2(c1, jets[i].M2(), jets[j].M2(), hadW.M2());
  ...
  {    
    ...
    if( btag[b] && b>2 ) continue;

    //require b-tagging if have more than 1 b-tag
    if( n_btag>1 && !btag[b] ) continue;
    ...
    for (unsigned int w=0;w<v_i.size();++w)
    {
      ...
      double pt_w1 = jets[k].Pt();
      double pt_w2 = jets[l].Pt();
      
      //  W Mass.
      TLorentzVector hadW = jets[k] + jets[l];
      double massW = hadW.M();
      ...
      // Top Mass.
      TLorentzVector hadT = (jets[k] * c1) + (jets[l] * c2) + jets[b];
      double massT = hadT.M();
      double pt_w = hadW.Pt();
...
extern "C" double ComputeChi2(int n_jets, double* jetPt, double* jetEta, double* jetPhi, double* jetE, bool* btag){
    Resolution res;
    TLorentzVector* jets = new TLorentzVector[n_jets];
    for(int i = 0; i<n_jets; i++){
        jets[i].SetPtEtaPhiE(jetPt[i],jetEta[i],jetPhi[i],jetE[i]);
    }
    return res.GetChi2(n_jets, jets, btag);
}
