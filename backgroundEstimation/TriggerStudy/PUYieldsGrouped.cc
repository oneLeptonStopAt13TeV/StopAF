...
//usage
//./PUYieldsGrouped signalRegPU.tab datacards/signalReg.txt
...
int main(int argc, char *argv[]){
        ...
        TFile *f = new TFile("/opt/sbg/scratch1/cms/mjansova/store/tmp/0909/signalMerged.root");
        f->ls();
        TH1D * h1 = (TH1D*)f->Get("hnvertex");
        double total = h1->Integral();
        double iLow = h1->Integral(1,20); //@MJ@ TODO integral and error
        //cout << "bin up to: " << h1->GetBinLowEdge(20) << endl;
        double iHigh = h1->Integral(21,50);
        ...
        vector<string> datasets = {"grouped"}; //@MJ@ TODO change the datasets to meaningful ones
	...
	string line;
       ...
       //can->cd();
       TFile fi2("PUYieldsGrouped2.root","RECREATE");
