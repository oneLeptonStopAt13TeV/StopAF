...
void BabyScrewdriver::ActionForEachEvent(string currentDataset)
{
    ...
    */
    ...
    for(uint32_t SR=0; SR<8; SR++) //@MJ@ TODO nr of sig regions changes
    {
        ...
        w = GetLumi() *  myEvent.scale1fb * myEvent.weight_lepSF*( nEvents / myEvent.wNormalization.at(28) )* myEvent.weight_PUdown; //@MJ@ TODO PU without any normalization?!
        weightV.push_back(w);
        //PUup
        if(counter == 1) statnames << "PUup"<< endl;
        w = GetLumi() *  myEvent.scale1fb * myEvent.weight_lepSF*( nEvents / myEvent.wNormalization.at(28) )* myEvent.weight_PUup;
        weightV.push_back(w);
    }
