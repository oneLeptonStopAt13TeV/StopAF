...
void DataCardProducer::WriteBinCard(const string& ofilename, int iBin){
	...
	//processes
	ofile<<"jmax "<<proc_names.size() -1<<" number of backgrounds ("; //subtract signal
	...
	for(unsigned int i=0;i<proc_names.size();i++){
		float nevents = 0;
		//if(proc_names[i] != "signal") @MJ@ TODO
			//retrieve nevents
			nevents = table.Get(bins_table_names[iBin],proc_table_names[i]).value();
			//nevents = table.Get(proc_table_names[iBin],bins_table_names[i]).value();
		ofile<<"\t"<<nevents;
	}
  	ofile<<endl;
	...
	for(unsigned int i=0;i<proc_names.size();i++){
		...
		if(proc_stat_buncert[i]){
			...
			Figure Npred = table.Get(bins_table_names[iBin],proc_table_names[i]);
